...
public class HeroPhysic : MonoBehaviour
{
	...
	void Start ()
	{
		...
		{
			if (t.name != hero.name && t.GetComponent<Collider>() && t.GetComponent<Rigidbody>())
			{
	...
	void StartRagdoll ()
	{
		...
		foreach (Transform t in bones)
		{
			if (t.GetComponent<Rigidbody>() && t != hero)
			{
				t.GetComponent<Rigidbody>().isKinematic = false;
				t.GetComponent<Collider>().isTrigger = false;
				t.GetComponent<Rigidbody>().drag = 0.0f;
	...
	void EndRagdoll ()
	{
		...
		foreach (Transform t in bones)
		{
			if (t != hero && t.GetComponent<Rigidbody>())
			{
				t.GetComponent<Rigidbody>().isKinematic = true;
				t.GetComponent<Collider>().isTrigger = true;
			}
