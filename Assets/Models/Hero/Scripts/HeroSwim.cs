...
public class HeroSwim : MonoBehaviour 
{
	...
	void WaterRagdoll ()
	{
		...
		foreach (Transform t in bones)
		{
			...
			{
				t.GetComponent<Rigidbody>().drag = waterDrag;
				t.GetComponent<Rigidbody>().AddForce( liftVector * 3f, ForceMode.Acceleration );
				t.GetComponent<Rigidbody>().AddTorque( Random.insideUnitSphere * 30f, ForceMode.Acceleration );
			}
		}
		
		// Recover from ragdoll ("Remove/Disable if the character has died")
		if (rootB.GetComponent<Rigidbody>().velocity.sqrMagnitude < 0.001f)
		{
			hPhys.end = true;
		}
	}
