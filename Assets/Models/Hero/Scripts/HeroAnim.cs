...
public class HeroAnim : MonoBehaviour 
{
	...
	Vector3 XZVelo 
	{
		get { return new Vector3 (GetComponent<Rigidbody>().velocity.x, 0.0f, GetComponent<Rigidbody>().velocity.z); }
	}
	...
	void Start ()
	{
        ...
        GetComponent<HeroMotor>().doCombatDel();
        //Invoke(GetComponent<HeroMotor>().doCombatDel, 1.0f);
        ////mainState = MainState.Combat;
        //// Combat stance if not already else Normal
        //if (mainState != HeroAnim.MainState.Combat)
        //    GetComponent<HeroMotor>().moveState = HeroMotor.MoveState.CombatStance;
        //// Combat state delegate
        //if (GetComponent<HeroMotor>().doCombatDel != null)
        //    GetComponent<HeroMotor>().doCombatDel();
